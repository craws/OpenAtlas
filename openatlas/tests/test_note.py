...
class NoteTest(TestBaseCase):
    ...
    def test_note(self):
        with app.app_context():
            ...
            rv = self.app.get(url_for('note_insert', entity_id=actor_id))
            assert b'Note *' in rv.data
            rv = self.app.post(url_for('note_insert', entity_id=actor_id),
            ...
            rv = self.app.get(url_for('note_update', entity_id=actor_id))
            assert b'A nice description' in rv.data
            rv = self.app.post(url_for('note_update', entity_id=actor_id),
                               data={'description': 'A very nice description'},
                               follow_redirects=True)
            assert b'Note updated' in rv.data
            rv = self.app.get(url_for('note_delete', entity_id=actor_id), follow_redirects=True)
