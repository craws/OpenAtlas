...
class ObjectTest(TestBaseCase):
    ...
    def test_object(self):
        with app.app_context():
            ...
            # Add to source
            rv = self.app.get(url_for('source_insert', origin_id=object_.id))
            assert b'A little hate' in rv.data
            rv = self.app.post(url_for('source_insert', origin_id=object_.id),
                               data={'name': 'Necronomicon', 'information_carrier': [object_.id]},
                               follow_redirects=True)
