...
const geoSearchControl = L.control.geonames({
    ...
    postalCodesRegex: /^([Gg][Ii][Rr] 0[Aa]{2})|((([A-Za-z][0-9]{1,2})|(([A-Za-z][A-Ha-hJ-Yj-y][0-9]{1,2})|(([A-Za-z][0-9][A-Za-z])|([A-Za-z][A-Ha-hJ-Yj-y][0-9]?[A-Za-z])))) [0-9][A-Za-z]{2})$/, // Regex used for testing user provided string for a postal code.  If this test fails, the default geonames API is used instead.
...
function setPopup(selected) {
    return (feature, layer) => layer.bindPopup(buildPopup(feature, 'view', selected));
}
