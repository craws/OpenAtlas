...
class LinkedPlaces:
    ...
    def get_entity(entity: Entity,
        ...
        return {
            ...
            'features': [{
                ...
                'relations': LinkedPlaces.get_relations(links, links_inverse, parser),
                'types': LinkedPlaces.get_types(entity, links, parser),
                'depictions': LinkedPlaces.get_depictions(links_inverse, parser),
                'when': LinkedPlaces.get_timespans(entity, parser),
                'links': LinkedPlaces.get_reference_links(links_inverse, parser),
                'description': [{'value': entity.description}] if entity.description else None,
    ...
    def get_types(
        ...
        return LPHelper.get_node(entity, links) if 'types' in parser['show'] else None

    @staticmethod
    def get_depictions(
            links_inverse: List[Link],
            parser: Dict[str, Any]) -> Optional[List[Dict[str, str]]]:
        return LPHelper.get_file(links_inverse) if 'depictions' in parser['show'] else None

    @staticmethod
    def get_timespans(entity: Entity, parser: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        return {'timespans': [LPHelper.get_time(entity)]} if 'when' in parser['show'] else None
    ...
    def get_crm_class(entity: Entity) -> str:
        return f"crm:{''.join(entity.cidoc_class.code + ' ' + entity.cidoc_class.i18n['en'])}"
