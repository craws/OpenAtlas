...
def settings_mail():
    ...
    return render_template(
        'settings/mail.html', settings=settings, mail_settings=mail_settings, form=form)
...
def settings_general():
    ...
    return render_template(
        'settings/general.html', settings=settings, general_settings=general_settings)
...
def settings_general_update():
    ...
    return render_template('settings/general_update.html', form=form, settings=session['settings'])


@app.route('/admin/mail/update', methods=["GET", "POST"])
@required_group('admin')
def settings_mail_update():
    form = MailForm()
    if form.validate_on_submit():
        g.cursor.execute('BEGIN')
        try:
    ...
    for field in SettingsMapper.fields:
        if field in ['mail_recipients_feedback']:
            ...
            getattr(form, field).data = session['settings'][field]
    return render_template('settings/mail_update.html', form=form, settings=session['settings'])
