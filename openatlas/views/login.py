...
class LoginForm(FlaskForm):
    ...
    password = PasswordField(str(_('password')), [InputRequired()])
    show_passwords = BooleanField(str(_('show password')))
    save = SubmitField(str(_('login')))


class PasswordResetForm(FlaskForm):
    email = StringField(str(_('email')), [InputRequired(), Email()])
    save = SubmitField(str(_('submit')))
    ...
    if form.validate_on_submit():
        ...
        if user and user.username:
            if user.login_attempts_exceeded():
                ...
                flash(str(_('error login attempts exceeded')), 'error')
            ...
            if hash_ == user.password.encode('utf-8'):
                ...
                flash(str(_('error inactive')), 'error')
            else:
                g.logger.log(
                    'notice',
                    'auth',
                ...
                user.update()
                flash(str(_('Invalid user or password')), 'error')
        else:
            g.logger.log(
                'notice',
                'auth',
    ...
    if form.validate_on_submit() and g.settings['mail']:
        if user := User.get_by_email(form.email.data):
            ...
            if send_mail(subject, body, form.email.data):
                flash(
                    str(_('A password reset confirmation mail was send '
                        'to %(email)s.', email=email)))
            else:  # pragma: no cover
                flash(
                    str(_('Failed to send password reset confirmation mail '
                        'to %(email)s.', email=email)),
                    'error')
        ...
        flash(str(_('error non existing email')), 'error')
    return render_template(
        'content.html',
...
def reset_confirm(code: str) -> Response:
    ...
    if not user or not user.username or not user.email:
        ...
        flash(str(_('invalid password reset confirmation code')), 'error')
        abort(404)
    ...
    if datetime.now() > user.password_reset_date + timedelta(hours=hours):
        ...
        flash(str(_('This reset confirmation code has expired.')), 'error')
        abort(404)
    ...
    if send_mail(subject, body, user.email, False):
        flash(
            str(_('A new password was sent to %(email)s.', email=user.email)))
    else:  # pragma: no cover
        flash(
            str(_(
                'Failed to send password mail to %(email)s.',
                email=user.email)),
            'error')
    return redirect(url_for('login'))
