...
def get_gis_by_id(id_: int) -> list[dict[str, Any]]:
    return db.get_by_id(id_)


def get_gis_by_entities(
...
def get_centroids_by_entities(
        entities: list[Entity]) -> dict[int, list[Any]]:
...
def get_gis_all(
    ...
    if structure:
        ...
        super_id = [structure['supers'][-1].id] if structure['supers'] else []
...
def insert_gis(entity: Entity, data: dict[str, Any]) -> None:
        ...
        for item in json.loads(data[shape]):
            ...
            db.insert(
                ...
                data={
                    ...
                    'description': sanitize(item['properties']['description']),
                    'type': item['properties']['shapeType'],
                    'geojson': json.dumps(item['geometry'])})


def delete_gis_by_entity(entity: Entity) -> None:
