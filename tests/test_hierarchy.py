...
class HierarchyTest(TestBaseCase):
    ...
    def test_hierarchy(self) -> None:
        with app.app_context():
            ...
            rv = self.app.get(
                url_for('required_add', id_=sex.id),
                follow_redirects=True)
            assert b'Changes have been saved' in rv.data
            ...
            rv = self.app.get(
                url_for('hierarchy_update', id_=get_hierarchy('Valued').id))
            assert b'Valued' in rv.data

            type_ = get_hierarchy('Actor relation')
            rv = self.app.get(url_for('hierarchy_update', id_=type_.id))
            assert b'Forbidden' in rv.data

            rv = self.app.get(url_for('hierarchy_delete', id_=type_.id))
            assert b'Forbidden' in rv.data
