...
class TypeTest(TestBaseCase):
    ...
    def test_type(self) -> None:
        ...
        rv = self.app.get(url_for('ajax_get_type_tree', root_id=actor_type.id))
        assert b'New dynamic' in rv.data

        rv = self.app.post(url_for('update', id_=actor_type.id), data=data)
        ...
        rv = self.app.get(url_for('show_multiple_linked_entities', id_=sex.id))
