...
class ObjectTest(TestBaseCase):
    ...
    def test_object(self) -> None:
        with app.app_context():  # type: ignore
            ...
            assert b'Love-letter' in rv.data
            rv = self.app.post(url_for('update', id_=artifact.id),
                               follow_redirects=True,
            ...
            rv = self.app.post(url_for('entity_add_source', id_=artifact.id),
                               data={'checkbox_values': str([source.id])},
                               follow_redirects=True)
            assert b'Necronomicon' in rv.data

            # Add to event
            rv = self.app.get(url_for('insert', class_='E9', origin_id=artifact.id))
            assert b'A little hate' in rv.data
            rv = self.app.post(url_for('insert', class_='E9', origin_id=artifact.id),
